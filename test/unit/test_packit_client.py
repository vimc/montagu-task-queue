...
def mock_auth(mock_montagu_api_class, requests_mock):
    ...
    requests_mock.get(f"{PACKIT_URL}/api/auth/login/montagu",
                      text=json.dumps({"token": "test-packit-token"}))


def assert_expected_packit_api_request(
        requests_mock,
        index,
        method,
        url,
...
def test_authenticates_on_init(MockMontaguAPI, requests_mock):
    ...
    MockMontaguAPI.assert_called_with(
        "http://test-montagu", "test.montagu.user", "montagu_password")
...
def test_refresh_git(MockMontaguAPI, requests_mock):
    ...
    assert_expected_packit_api_request(
        requests_mock, 1, "POST", f"{PACKIT_URL}/api/runner/git/fetch")
...
def test_run(MockMontaguAPI, requests_mock):
    ...
    requests_mock.get(f"{PACKIT_URL}/api/runner/git/branches",
                      text=json.dumps(mock_branches_response))
    requests_mock.post(f"{PACKIT_URL}/api/runner/run",
                       text=json.dumps({"taskId": "test-task-id"}))
    ...
    assert_expected_packit_api_request(
        requests_mock, 1, "GET", f"{PACKIT_URL}/api/runner/git/branches")
    ...
    assert_expected_packit_api_request(
        requests_mock, 2, "POST",
        f"{PACKIT_URL}/api/runner/run", expected_run_payload)
...
def test_poll_status(MockMontaguAPI, requests_mock):
    ...
    requests_mock.get(f"{PACKIT_URL}/api/runner/status/test-task-id",
                      text=json.dumps(mock_poll_response))
...
def test_kill_task(MockMontaguAPI, requests_mock):
    ...
    requests_mock.post(f"{PACKIT_URL}/api/runner/cancel/test-task-id",
                       text=json.dumps(mock_kill_response))
    ...
    assert_expected_packit_api_request(
        requests_mock, 1, "POST",
        f"{PACKIT_URL}/api/runner/cancel/test-task-id")


@patch("montagu.MontaguAPI")
def test_publish(MockMontaguAPI, requests_mock):
    mock_auth(MockMontaguAPI, requests_mock)
    # publish polls for the packit - return 404 on first poll,
    # 200 on second poll
    requests_mock.get(f"{PACKIT_URL}/api/packets/test-packet-id", [
        {"text": json.dumps({"error": {"detail": "not found"}}),
         "status_code": 404},
        {"text": json.dumps(
            {"id": "test-packet-id", "name": "A packet"}), "status_code": 200}
    ])
    expected_permissions_payload = json.dumps({
        "addPermissions": [
            {"permission": "packet.read", "packetId": "test-packet-id"}
        ],
        "removePermissions": []
    })
    requests_mock.put(
        f"{PACKIT_URL}/api/roles/test-role-1/permissions", json="null")
    requests_mock.put(
        f"{PACKIT_URL}/api/roles/test-role-2/permissions", text="null")

    sut = PackitClient(config)
    result = sut.publish("A packet", "test-packet-id",
                         ["test-role-1", "test-role-2"])
    assert_expected_packit_api_request(
        requests_mock, 1, "GET",
        f"{PACKIT_URL}/api/packets/test-packet-id",
        None)
    assert_expected_packit_api_request(
        requests_mock, 2, "GET",
        f"{PACKIT_URL}/api/packets/test-packet-id",
    ...
    assert_expected_packit_api_request(
        requests_mock, 3, "PUT",
        f"{PACKIT_URL}/api/roles/test-role-1/permissions",
        expected_permissions_payload)
    assert_expected_packit_api_request(
        requests_mock, 4, "PUT",
        f"{PACKIT_URL}/api/roles/test-role-2/permissions",
        expected_permissions_payload)
    assert result


@patch("montagu.MontaguAPI")
def test_sets_auth_success_to_false_when_auth_fails(
        MockMontaguAPI,
        requests_mock):
    requests_mock.get(f"{PACKIT_URL}/api/auth/login/montagu",
                      status_code=401,
...
def test_reauthenticates_on_401(MockMontaguAPI, requests_mock):
    ...
    # with all methods which require authentication - here we just test a
    # sample method to check the pattern works.
    mock_auth(MockMontaguAPI, requests_mock)
...
def test_raises_exception_on_unexpected_status(
        ...
        requests_mock):
    mock_auth(MockMontaguAPI, requests_mock)
    # execute does not tolerate status codes other than 401
    # - should get an exception
    bad_response = {"error": "Bad request"}
    requests_mock.get(f"{PACKIT_URL}/api/runner/status/test-task-id",
                      status_code=400, text=json.dumps(bad_response))
